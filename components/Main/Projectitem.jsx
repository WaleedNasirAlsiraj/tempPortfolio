...
const Projectitem = ({ src, title, description, link }) => {
  ...
  return (
    <PinContainer
      title={title}
      href={link}
      className={"w-full h-full  md:inline"}
    >
      <motion.div
        animate={{
          y: hover ? -3 : 0,
        ...
        className="w-full h-full hover:shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] transition-all duration-200 cursor-pointer"
      >
        <Card className="p-3 lg:p-10 flex flex-row lg:flex-col justify-between items-center bg-secondary-foreground/5">
          <div className="flex flex-row lg:flex-col gap-4 w-full justify-start items-center lg:items-start">
            <div className="lg:w-full ">
              <img
                src={src}
                className="p-2 lg:p-0 lg:w-full bg-secondary/40 rounded-full lg:rounded-2xl"
              />
            </div>
            <div>
